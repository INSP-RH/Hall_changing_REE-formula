...
child_weight <- function(age, sex, bmiCat, FM = child_reference_FFMandFM(age, sex, bmiCat)$FM, 
  ...
  if (is.na(EI[1]) & (is.na(richardsonparams$K) || is.na(richardsonparams$Q) || 
    ...
    EI <- child_reference_EI(age, sex, bmiCat, FM, FFM, days, dt) 
  }
  ...
  if (!is.na(EI[1])){
    ...
    wt <- child_weight_wrapper(age, newsex, bmiCat, FFM, FM, as.matrix(EI), days, dt, checkValues)  
  } else {
